...
#Function to compute Term Frequency(TF = no. of times word present in page/no. of words)
def tf(word, page, pagelen):
    return (float)(page.words.count(word)) / pagelen

#Function to compute the number of pages a word is present in
def n_containing(word, commentList):
    return sum(1 for page in commentList if word in page)
...
def tfidf(word, page, commentList):
    pagelen = (float)(len(page.words))
    return tf(word, page, pagelen) * idf(word, commentList)
...
def preprocessing(commentFull):
    ...
    commentFull = re.sub(r'r/\S+', '', commentFull)

    #Expanding contractions in text
    commentFull = expandContractions(commentFull)
